...
*/
void makepair ( boy b[] , girl g[] )
{
	time_t now;	
	int pos;
	int i,j,MaxA,MaxB,MaxI;
	freopen ( "couple2.txt" , "w" , stdout );
	for ( i=1 ; i < gsize-1 ; i++ )
	{
		...
		switch ( cr )
		{
			case 0 : {
				...
				if ( b[pos].getBudget() >= g[i].getMaintenance() && b[pos].getMinattr() <= g[i].getAttractiveness() )
				{
					g[i].setCommitted(1);
			...
			case 2 : {
				...
				if ( b[pos].getBudget() >= g[i].getMaintenance() && b[pos].getMinattr() <= g[i].getAttractiveness() )
				{
					...
					now = time(0);
					string dt = ctime(&now);					
					cout<<g[i].getName()<<" and "<<b[pos].getName()<<" are commited "<<dt;
					break;
				}
				break;
			}
